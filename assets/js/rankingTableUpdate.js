...
(function($){
	...
	$.fn.rankingTableUpdate = function(newTable, options){
		...
		//jNewTable.hide();
        $(origTable).css('z-index', '1000');
        $(jNewTable).css({'z-index':'0', 'position':'absolute', 'top': 0, 'left': -100000});

        // Удаляем строки, которые скрыты, чтобы не было проблем при анимации
        $(origTable).find('tr.hidden').remove();
        $(jNewTable).find('tr.hidden').remove();
        ...
        var origTBodyHeight = $(origTBody).height();
        var newTBodyHeight = $(newTBody).height();

        if (newTBodyHeight > origTBodyHeight) {
            var addHeight = newTBodyHeight - origTBodyHeight;
            $('#vgs-streams-table').css('height', $('#vgs-streams-table').outerHeight() + addHeight + 'px');
        }
		...
		if(numRowsStaying === rowsInNewTable && noUpdatingColumns){
			...
			setTimeout(function(){
				...
				jNewTable.show();
                jNewTable.css({'position':'relative', 'left':0});
                $(jNewTable).parents('#vgs-streams-table').css('height', 'auto');
		...
		$.each(fresh, function(row){
            ...
            var height = returnHeight( $(newTBody.rows[row]) );
		...
		jOrigTable.find('div.moveable').each(function(i, wrapper){
			...
			if(row in up){
				...
				//move the row up..
				var topDiff = $(newTBody.rows[up[row]]).position().top  - $(origTBody.rows[row]).position().top;
				...
				//move the row down..
				var topDiff = $(newTBody.rows[down[row]]).position().top - $(origTBody.rows[row]).position().top;
				secondAnimator.addSubject(new NumericalStyleSubject(wrapper, "top", 0, topDiff));
				...
				//move the row up..
				var topDiff = $(newTBody.rows[row]).position().top - $(wrapper.parentNode).position().top;
				secondAnimator.addSubject(new NumericalStyleSubject(wrapper, "top", 0, topDiff));						
